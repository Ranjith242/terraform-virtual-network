...
# ---------- Helper: derive a short suffix from RG name ----------
# Strips a leading "rg-" if present, then keeps the rest (lower-case, alphanumerics).
...
# Lower-case alphanumeric only (for storage account names)
...
$envs = @(
    @{
        ...
        Suffix       = Get-EnvSuffix $ProdResourceGroupName    # e.g. finops-prod
    ...
    @{
        ...
        Suffix       = Get-EnvSuffix $NonProdResourceGroupName  # e.g. finops-nonprod
...
foreach ($e in $envs) {
    ...
    $tag      = $e.Tag           # prod / nonprod (used for VM names)
    $suffix   = $e.Suffix        # e.g. finops-prod
    $rg       = $e.Rg
    $vnetName = $e.Vnet
    ...
    # ---------- Storage account (must be 3-24 chars, lower alphanumeric, globally unique) ----------
    ...
    Write-Host "  Creating App Service Plan $planName + Web App $webName ..." -ForegroundColor Cyan
    New-AzAppServicePlan -ResourceGroupName $rg -Name $planName -Location $Location -Tier Basic -NumberofWorkers 1 -WorkerSize Small | Out-Null
    New-AzWebApp        -ResourceGroupName $rg -Name $webName  -Location $Location -AppServicePlan $planName | Out-Null
